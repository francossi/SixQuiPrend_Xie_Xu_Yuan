...
public class GUI extends Application {
    ...
    Deck deck = new Deck(); // 创建一副牌 Créer un jeu de cartes
...
//        }
    ...
    private void handleCardClick(Card card) {
        ...
        if (closestRowCards != null && closestRowCards.size() > 5) {  // 注意这里检查了 closestRowCards 是否为 null
            ...
            System.out.println("The first five cards have been included in the player's pile, now "+sixthCard.getNumber()+" is the first card");
...
//            System.out.println("You have played a card: " + card.getNumber());
    ...
    private void MinimumValueCaseComputer(Card card){
        ...
        System.out.println("Because this card is so weak that it cannot go into any of the series. And the robot has chosen to add the card to row "+ chosenRow);
        List<Card> rowCards = middleCards.get(chosenRow);
    ...
    private void handleComputerPlayerTurn() {
        ...
        if (closestRowCards != null && closestRowCards.size() > 5) {  // 注意这里检查了 closestRowCards 是否为 null
            ...
            // 把第六张卡作为新的第一张卡
            closestRowCards.add(sixthCard);
            System.out.println("The first five cards have been included in the computer's pile, now "+sixthCard.getNumber()+" is the first card");
            Timeline timeline2 = new Timeline(new KeyFrame(Duration.seconds(1), event1 -> {
                updateMiddleCardRows();
                computerPlayerBullheadsLabel.setText(computerPlayer.getName() + "'s cattle heads:" + computerPlayer.getBullheads());
            }));
